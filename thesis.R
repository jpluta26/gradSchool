...
# ----------------------------- functions -------------------------------- #
# ------------------------------------------------------------------------ #
covTestVerbose=function (fitobj, x, y, sigma.est = "full", status = NULL, maxp = min(nrow(x), 
                                                                      ncol(x))) 
{
  # cov[j] - cov0[j] is the drop in covariance statistic
  ...
  # vector of cov stats
  tt = ((cov - cov0)/sigma^2)
  #print(paste("step ", j, ", cov stat = ", tt))
  if (sigma.type == "known") {
...
fit <- glm(cohort ~ CA1.l + Age + Gender + ICV, data=data[data$side=="left",], family='binomial')
